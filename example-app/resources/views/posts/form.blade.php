<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            {{ isset($post) ? 'Обновить пост' : 'Создание поста' }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="post" action="{{ isset($post) ? route('posts.update', $post->id) : route('posts.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="$post->title ?? old('Заголовок')" required autofocus />
                        ...
                        <div>
                            ...
                            <x-textarea-input id="content" name="content" class="mt-1 block w-full" required autofocus>{{ $post->content ?? old('Контент') }}</x-textarea-input>
                            <x-input-error class="mt-2" :messages="$errors->get('content')" />
                        </div>

                        <div>
                            <x-input-label for="featured_image" value="Изображение" />
                            <label class="block mt-2">
                                <span class="sr-only">Выбрать изображение</span>
                            ...
                            <div class="shrink-0 my-2">
                                <img id="featured_image_preview" class="h-64 w-128 object-cover rounded-md" src="{{ isset($post) ? asset($post->featured_image) : '' }}" alt="Вывод изображения" />
                            </div>
